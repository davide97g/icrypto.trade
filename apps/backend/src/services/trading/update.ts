...
export const tryUpdate = (strategy: Strategy) => {
  ...
  if(timeSinceLastOCOUpdate > 1000 * 10) {
    ...
    if(strategy.stats.variable.eventTime < kline.closeTime) firstVolume = firstVolume * 60*1000 / (strategy.stats.variable.eventTime % (60 * 1000));
    //we need to adjust the volume used based on the time of the news
    if(news && news.time) {
      const timeNewsKline = (strategy.stats.variable.eventTime < kline.closeTime ? strategy.stats.variable.eventTime : kline.closeTime) - news!.time;
    ...
    if(strategy.stats.variable.lastVolume > 0) {
      const currentVolume = (strategy.stats.variable.eventTime % (60 * 1000)) > 10 *1000 ? 
                            strategy.stats.variable.lastVolume * 60*1000 / (strategy.stats.variable.eventTime % (60 * 1000)) :
    ...
    const previousTPPercentage = (strategy.lastOcoOrderRequest.takeProfitPrice - strategy.stats.variable.lastPrice) / strategy.stats.variable.lastPrice;
    const previousSLPercentage = (strategy.stats.variable.lastPrice - strategy.lastOcoOrderRequest.stopLossPrice) / strategy.stats.variable.lastPrice;
    needsUpdate = 
      takeProfitPercentage/previousTPPercentage < 0.8 || takeProfitPercentage/previousTPPercentage > 1.2 ||
      stopLossPercentage/previousSLPercentage < 0.8 || stopLossPercentage/previousSLPercentage > 1.2;
    //and the min adjustment is 0.5%
    needsUpdate = needsUpdate && (Math.abs(previousTPPercentage - takeProfitPercentage) > 0.005 || Math.abs(previousSLPercentage - stopLossPercentage) > 0.005);
  ...
  console.info("The last TP was ", strategy.lastOcoOrderRequest.takeProfitPrice, "and the new TP is", TP);
